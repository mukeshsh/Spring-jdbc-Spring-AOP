...
public class CommonExceptionLoggerAspect {
     ...
     static {
		 ...
		 * logger.error(ex+"   exception is raised in     "+jp.getSignature()
		 * +"      method with args      "+Arrays.deepToString(jp.getArgs())); } public
		 * void exceptionLogger(JoinPoint jp,Throwable ex) {
		 * System.out.println("------2------");
		 * logger.error(ex+"   exception is raised in     "+jp.getSignature()
		 * +"      method with args      "+Arrays.deepToString(jp.getArgs())); }
		 */
     public void exceptionLogger(JoinPoint jp,Throwable ex) {
