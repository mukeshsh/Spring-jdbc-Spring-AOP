...
@Repository("bookStoreDAO")
   public class BookStoreDAOImpl implements BookStoreDAO {
	private static final String  GET_BOOKS_BY_AUTHORS=
			"SELECT BOOKID,BOOKNAME,AUTHOR,PRICE,PUBLISHER,STATUS,CATEGORY FROM BOOK_DETAILS WHERE AUTHOR IN(:auth1,:auth2,:auth3) ORDER BY AUTHOR";
	private static final String  INSERT_BOOK_STORE_QUERY=
			"INSERT INTO BOOK_DETAILS VALUES(BKID_SEQ.NEXTVAL,:bookName,:author,:price,:publisher,:status,:category)";
	
	@Autowired
	private NamedParameterJdbcTemplate npjt;

	@Override
	public List<BookBO> findBooksByAuthor(String author1, String author2, String author3) {
		 ...
		 * //method
		 * 
		 * @Override public int insertBook(BookBO bo) { MapSqlParameterSource msps=null;
		 * int count=0; //prpeare param values msps=new MapSqlParameterSource();
		 * msps.addValue("bookName", bo.getBookName());
		 * msps.addValue("author",bo.getAuthor()); msps.addValue("price",
		 * bo.getPrice()); msps.addValue("publisher",bo.getPublisher());
		 * msps.addValue("status", bo.getStatus());
		 * msps.addValue("category",bo.getCategory());
		 * count=npjt.update(INSERT_BOOK_STORE_QUERY, msps); return count; }
		 */
